...
if(F){
  ...
  transect_plot <- function(transect = NULL, n=25){
    ...
    wind <- ggplot(d) +
      ...
      scale_color_manual(values=c("dodgerblue", "red"), drop=F) +
      annotate(geom="text", 
...
# pal <- c("black", "black", "black", "darkmagenta", "#6b49ff", 
#          "dodgerblue", "turquoise", "#48c13f", 
#          "gold", "red", "#660000")
pal <- c("#ebfeff", "#ebfeff", "#ebfeff", "#defeff", "#d1feff", "#d1feff", 
         "cyan", "dodgerblue", "dodgerblue", 
         "#4063ff", "#b640ff", "magenta",
...
map <- ggplot() +
  ...
  shadowtext::geom_shadowtext(data=dp %>% filter(! i %in% 1:2), 
            aes(x, y-7, label=plett[i]), color="black", size=4, bg.color = "white") +
  ...
  shadowtext::geom_shadowtext(data=dp_circ, aes(x, y-7, label=plett[i]), color="black", size=4, bg.color = "white") +
...
latitude <- ggplot(vs %>% filter(land=="land")) +
  ...
  scale_color_manual(values=c("red", "dodgerblue"), drop=F)  +
  scale_fill_manual(values=c("red", "dodgerblue"), drop=F)  +
...
ggs("figures/manuscript/fig_2.pdf", pc, width=9, height=7, units="in", 
