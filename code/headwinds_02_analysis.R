...
# long-term means
t <- mf[grepl("tmp2m", mf)] %>% stack() %>% mean()
u <- mf[grepl("_u", mf)] %>% stack() %>% mean()
v <- mf[grepl("_v", mf)] %>% stack() %>% mean()
...
vx1 <- rotate(-1 * focal(t, w=matrix(1,3,3), fun=function(x) gradient(x, component="x")))
vx2 <- -1 * focal(rotate(t), w=matrix(1,3,3), fun=function(x) gradient(x, component="x"))
vx <- merge(vx1, vx2)
vy1 <- rotate(focal(t, w=matrix(1,3,3), fun=function(x) gradient(x, component="y")))
vy2 <- focal(rotate(t), w=matrix(1,3,3), fun=function(x) gradient(x, component="y"))
vy <- merge(vy1, vy2)
vx <- trim(vx)
vy <- crop(vy, vx)
...
cr <- function(x) crop(rotate(x), tm)
...
png("figures/temperature_direction.png", width=2000, height=1000)
plot(map)
...
map <- ggplot() +
      ...
      theme(legend.position="none",
            panel.background=element_rect(fill="black"))
png("figures/wind_direction.png", width=2000, height=1000)
plot(map)
plot(compass, vp=viewport(x=.13, y=.33, width=.25, height=.35))
dev.off()
...
ggsave("figures/scatter_azimuth_magnitude.png", p, width=8, height=8, units="in")
...
ggsave("figures/scatter_azimuth_magnitude_all.png", p, width=8, height=8, units="in")
...
map <- ggplot() +
      ...
      theme_void() +
      theme(legend.position="none")
png("figures/wind_direction_full.png", width=2000, height=1000)
plot(map)
plot(compass, vp=viewport(x=.13, y=.33, width=.25, height=.35))
dev.off()
...
map <- ggplot() +
      ...
      theme(legend.position="none",
            panel.background=element_rect(fill="black"))
png("figures/alignment.png", width=2000, height=1000)
plot(map)
...
p <- ggplot(dl, aes(theta)) +
      ...
      theme_minimal()
ggsave("figures/ecdf_theta.png", p, width=8, height=8, units="in")
...
p <- ggplot(dl, 
      ...
      theme_minimal() +
      labs(x="latitude")
ggsave("figures/scater_latitude_theta.png", p, width=8, height=8, units="in")
